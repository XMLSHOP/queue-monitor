    ...
    <form action="" method="GET">
        <div class="flex items-center my-2 -mx-2">
            ...
            <div class="px-2 w-1/4">
                <label for="filter_schedulers" class="block mb-1 text-xs uppercase font-semibold text-gray-600">
                    @lang('Schedulers')
                </label>
                <select name="scheduler" id="filter_schedulers"
                        class="w-full p-2 bg-gray-200 border-2 border-gray-300 rounded">
                    <option value="all">All</option>
                    @foreach($schedulers_list as $scheduler)
                        <option @if((int)$filters['scheduler'] == $scheduler['id']) selected @endif value="{{ $scheduler['id'] }}">
                            {{ $scheduler['name'] }}
                        </option>
                    @endforeach
                </select>
