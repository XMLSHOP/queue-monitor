<h1 class="mb-6 text-5xl text-blue-900 font-bold">
    @lang('Monitor'): @lang('Schedulers')
</h1>

@include('monitor::schedulers.partials.filters', [
    'filters'=>$filters
])
            ...
            <tr class="font-sm leading-relaxed">
                ...
                <td class="p-4 text-gray-800 text-sm leading-5 font-medium border-b border-gray-200">
                    <div class="ml-1 text-xs text-gray-600">
                        ...
                        <span class="font-semibold">{{ $scheduler->host->alias ?? $scheduler->host->name }}</span>
                    </div>
                </td>

                <td class="p-4 text-gray-800 text-sm leading-5 border-b border-gray-200">
                    <div class="ml-1 text-xs text-gray-1000">
                        Started: <strong>{{ $scheduler->getStarted() }}</strong>
                        @if($scheduler->getFinished())
                            <div>Finished: <strong>{{ $scheduler->getFinished() }}</strong></div>
                            <div style="height: 5px"></div>
                            <div>Duration:
                                <strong>
                                    @if($scheduler->getElapsedInterval()->format('%H:%I:%S') == '00:00:00')
                                        < 1s
                                    @else
                                        {{ $scheduler->getElapsedInterval()->format('%H:%I:%S') }}
                                    @endif
                                </strong>
                            </div>
                        @endif
                ...
                <td class="p-4 text-gray-800 text-sm leading-5 border-b border-gray-200">
                    ...
                    @if($scheduler->hasFailed() && $scheduler->exception !== null)
                        <textarea rows="4" class="w-64 text-xs p-1 border rounded form-control" readonly
                                  style="resize: both"
                        >
                            {{ $scheduler->exception->exception_message }}
                        </textarea>
                    @else
                        -
                    @endif
        ...
        <tr>
            ...
            <td colspan="100" class="px-6 py-4 text-gray-700 font-sm border-t-2 border-gray-200">
                ...
                <div class="flex justify-between">
                    ...
                    <div>
                        @lang('Showing')
                        @if($schedulers->total() > 0)
                            <span class="font-medium">{{ $schedulers->firstItem() }}</span> @lang('to')
                            <span class="font-medium">{{ $schedulers->lastItem() }}</span> @lang('of')
                        @endif
                        <span class="font-medium">{{ $schedulers->total() }}</span> @lang('result')
                    </div>

                    <div>

                        <a class="py-2 px-4 mx-1 text-xs font-medium @if(!$schedulers->onFirstPage()) bg-gray-200 hover:bg-gray-300 cursor-pointer @else text-gray-600 bg-gray-100 cursor-not-allowed @endif rounded"
                           @if(!$schedulers->onFirstPage()) href="{{ $schedulers->previousPageUrl() }}" @endif>
                            @lang('Previous')
                        </a>

                        <a class="py-2 px-4 mx-1 text-xs font-medium @if($schedulers->hasMorePages()) bg-gray-200 hover:bg-gray-300 cursor-pointer @else text-gray-600 bg-gray-100 cursor-not-allowed @endif rounded"
                           @if($schedulers->hasMorePages()) href="{{ $schedulers->url($schedulers->currentPage() + 1) }}" @endif>
                            @lang('Next')
                        </a>
